// Enhance the sidebar component for better desktop experience
"use client"
...
const SidebarProvider = ({ open: defaultOpen, onOpenChange: defaultOnOpenChange, children }: SidebarProviderProps) => {
  ...
  return <SidebarContext.Provider value={{ open, onOpenChange }}>{children}</SidebarContext.Provider>
}
...
const sidebarVariants = cva(
  "fixed left-0 top-0 z-50 h-full w-64 lg:w-72 border-r bg-background transition-transform duration-300 ease-in-out md:translate-x-0",
  {
    variants: {
      open: {
        true: "translate-x-0",
...
const sidebarMenuButtonVariants = cva(
  "group relative flex w-full items-center space-x-2 rounded-md p-2 text-sm font-medium hover:bg-secondary hover:text-foreground transition-colors",
  {
    variants: {
      isActive: {
